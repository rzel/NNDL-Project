...
class HighwayLayer(object):
    def __init__(self, rng, input, n_in, n_out, W_H=None, b_H=None, W_T =None, b_T = None,
        ...
        self.gate_output = activation_T(T_part)
        self.block_output = activation_H(H_part)
        self.output = self.block_output*self.gate_output + input*(one-self.gate_output)
        # parameters of the model
...
class HighwayNetwork(object):
    def __init__(self, rng, input, n_in, n_hidden, n_out, n_hiddenLayers, n_highwayLayers, activation_hidden, activation_highway, b_T):#, training_enabled):
        ...
        self.gate_outputs = [x.gate_output for x in self.allLayers[1:]]
        self.block_outputs = [x.output for x in self.allLayers[1:]]
...
class train_result(object):
    ...
    """
    def __init__(self, RunningTime, XEntropy, TestPerformance,
                 BestValidationScore, N_Epochs, N_Iterations, Patience,
                 Gate_out, Block_out):
        ...
        self.Gate_outputs = Gate_out
        self.Block_outputs = Block_out

def train_nn(train_model, validate_model, test_model, 
            n_train_batches, n_valid_batches, n_test_batches, n_epochs, 
            gate_output, block_output, verbose = True, early_stopping=True):
    """
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in range(n_train_batches):
            ...
            gate_out = gate_output(minibatch_index)
            block_outputs = block_output(minibatch_index) 
            
            if n_epochs-epoch<400:
                gate_outputs[epoch] = gate_out            
            ...
            if (iter + 1) % validation_frequency == 0 and early_stopping:
            ...
            if patience <= iter and early_stopping:
                done_looping = True
                break
        if verbose:
            print(epoch,epoch_cross_entropy/n_train_batches)        
        cross_entropy[epoch] = epoch_cross_entropy/n_train_batches
        
        if numpy.isnan(epoch_cross_entropy/n_train_batches):
            done_looping=True
    ...
    return train_result(
            ...
            Patience = patience,
            Gate_out = (gate_outputs,gate_out),
            Block_out = block_outputs
        ) 

def load_data(dataset,reduced=1):
    ''' Loads the dataset
    :type dataset: string
    ...
    def shared_dataset(data_xy, reduced, borrow=True):
        ...
        if reduced!=1:
            pct = int(data_x.shape[0]*reduced)
            data_x, data_y = data_x[:pct], data_y[:pct]
    ...
    test_set_x, test_set_y = shared_dataset(test_set,reduced)
    valid_set_x, valid_set_y = shared_dataset(valid_set,reduced)
    train_set_x, train_set_y = shared_dataset(train_set,reduced)
